  ...
  return (
    <div
      style={{
        ...
        fontSize: 13,
        color: secondsLeft <= 10 ? "var(--danger)" : "#92400E",
        fontWeight: 500,
      }}
    >
      <span>
        ⏱ Admin session expires in <strong>{secondsLeft}s</strong> due to
        inactivity
      </span>
      <span style={{ fontSize: 11, opacity: 0.7 }}>
        Move mouse or type to reset
      </span>
    </div>
  );
}
...
export default function AdminPage() {
  ...
  const resetTimer = useCallback(() => {
    ...
    setSecondsLeft(IDLE_TIMEOUT_MS / 1000);
  }, []);
  ...
  useEffect(() => {
    const tick = () => {
      const remaining = Math.max(
        0,
        IDLE_TIMEOUT_MS - (Date.now() - lastActivity.current),
      );
  ...
  const [activeTab, setActiveTab] = useState<"products" | "manpower">(
    "products",
  );
  ...
  const saveRate = async () => {
    ...
    await supabase
      .from("settings")
      .upsert(
        { key: "manpower_daily_rate", value: manpowerRate.toString() },
        { onConflict: "key" },
      );
    setRateSaving(false);
  ...
  const handleSave = async () => {
    if (!form.brand_model.trim()) {
      showAlert("error", "Please enter a brand/model name.");
      return;
    }
    ...
    if (editing) {
      const { error } = await supabase
        ...
        .eq("id", editing.id);
      if (error) showAlert("error", "Error saving.");
      else showAlert("success", "Product updated.");
    } else {
      const { error } = await supabase.from("products").insert([payload]);
      if (error) showAlert("error", "Error creating.");
      else showAlert("success", "Product added.");
    }
    setSaving(false);
    setShowModal(false);
    await load();
  ...
  const filtered = products.filter((p) => {
    ...
    const sr =
      !search || p.brand_model.toLowerCase().includes(search.toLowerCase());
    return sm && sr;
  });
              ...
              <table className="data-table">
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id}>
                      <td>
                        <span className="badge badge-section">{p.section}</span>
                      </td>
      ...
      {activeTab === "manpower" && (
        <div className="card" style={{ padding: 28, maxWidth: 500 }}>
          <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>
            Default Manpower Daily Rate
          </div>
          <div style={{ fontSize: 13, color: "var(--gray)", marginBottom: 20 }}>
            This sets the default daily rate per person pre-filled when creating
            a new offer. It can still be overridden per offer.
          </div>
          {rateLoading ? (
            ...
            <>
              <div className="form-group" style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  value={manpowerRate}
                  onChange={(e) =>
                    setManpowerRate(parseFloat(e.target.value) || 0)
                  }
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    f("price_per_unit", parseFloat(e.target.value) || 0)
                  }
                />
              </div>
              <div className="form-group full">
                ...
                <input
                  ...
                  placeholder="e.g. Huawei | LUNA2000-7-S1"
                />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input
                  value={form.colour ?? ""}
                  onChange={(e) => f("colour", e.target.value)}
              ...
              <div className="form-group">
                ...
                <input
                  value={form.garantie ?? ""}
                  onChange={(e) => f("garantie", e.target.value)}
                  placeholder="e.g. 10 or 25 / 30"
                />
              </div>
              <div className="form-group full">
                ...
                <input
                  value={form.usp ?? ""}
                  onChange={(e) => f("usp", e.target.value)}
                  placeholder="e.g. Includes backup power, SG-Ready"
                />
              </div>
            </div>
