...
function defaultState(defaultRate = 0): OfferFormState {
  return {
...
export function OfferFormProvider({ children }: { children: ReactNode }) {
  ...
  const resetForm = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await supabase
        ...
        .single();
      if (data?.value) defaultRate = parseFloat(data.value) || 0;
    } catch {}
    const fresh = defaultState(defaultRate);
    setFormState(fresh);
    try {
  ...
  const loadFromOffer = useCallback((offer: any, newVersion: number) => {
    ...
    if (Array.isArray(offer.items)) {
      ...
      SECTIONS.forEach((s) => {
        if (bySec[s]) sectionStates[s] = bySec[s];
      });
    }
  ...
  return (
    <OfferFormContext.Provider
      value={{ form, setForm, updateForm, resetForm, loadFromOffer }}
    >
      {children}
    </OfferFormContext.Provider>
  );
